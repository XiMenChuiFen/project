...
if(empty($english) || empty($chinese1) || empty($chinese2)  || empty($type)){ //|| empty($chinese3)
    $message=[
        'mes'=>'还有空项未填',
        'number'=>404,
    ];
...
//|| !preg_match("/^[\x7f-\xff]+$/", $chinese3)
  ...
  // $type=strFilter($type);
   $sql = "SELECT * FROM word WHERE  english='$english' and user ='$user' LIMIT 1";
        ...
        if($data){
           ...
           $in="INSERT INTO word VALUES ('','$english','$chinese1','$chinese2','$user','$type','$time','$encryption')";
