      ...
      <el-main class="word-data-box">
        <el-card class="box-card">
            <div slot="header" class="clearfix e-title">
                ...
                <el-tooltip class="item" effect="dark" content="在线翻译" placement="top-start">
                  <i class="el-icon-tickets word-icon icon"><router-link :to="'/Translate/'+word.english" class="translate-a"></router-link></i>
                </el-tooltip>
                <el-tooltip class="item" effect="dark" content="发音" placement="top-start">
                 ...
                 <!-- <el-tooltip class="item" effect="dark" content="收藏" placement="top-start"> -->
                    ...
                    <i class="el-icon-star-on word-icon icon"  :class="{icon2:word.collection=='1'}" @click="mycollection(word.id,index)">
                    </i>
                    <!-- <span>{{word.id}}{{index}}</span> -->
                <!-- </el-tooltip> -->
            </div>
...
//   name: 'app',
    data(){
        return{
            data:[],//单词原始数据
           datas:[],//单词输出页面数据
           //dats输出到页面修改过，data从数据库和获取到未修改过(但收藏单词是会修改里面的collection)
           wordtitle:'全部单词',//表头
           wordlength:'',//单词数量
           url:'',//发音链接
           audio:'',//音频
    ...
    methods:{
        ...
        getword(){//获取所有的单词数据
            ...
            connect.$on('username', function(username){//获取用户名
                th.data=[],//单词原始数据
                th.datas=[],//单词输出页面数据
                // console.log(id,username);
                th.username=username;//用户名
                this.$http.get('/getword').then((res)=>{//获取全部单词
                    ...
                    th.$http.get("/getmycollection?username="+th.username).then((res)=>{//获取已收藏单词
                        // wordColle=[];
                        ...
                        for (let index = 0; index < allword.data.length; index++) {
                            ...
                            if(wordColle.indexOf(allword.data[index].id) >= 0 ){//改变数据结构并且为已经收藏的单词加识别
                                word={
                                    ...
                                    colluser:res.data[0].username,//添加收藏者名
                                }
                            }else{
                                word={
                                    id:allword.data[index].id,
        ...
        getuserword(){//取筛单词后输出
            let th=this;
            connect.$on('userworddata',function(username){//userNav数据
                // console.log(username,'姓名');
                // console.log(th.data,th.data);//dats输出到页面，data从数据库和获取到未修改过
                let userword=[];
                if(username=="全部成员"){
                    th.wordtitle="全部单词";
                    th.wordlength=th.data.length;
                    if(th.data.length==0){
                        th.isdatames=true;
                        th.data=[];
                    }else{
                ...
                }else if(username=="我的收藏"){//我的收藏
                    th.wordtitle=`${username}`;
                    // console.log(th.username);
                    for(let i=0;i<th.data.length;i++){
                        if(th.data[i].collection==1 && th.data[i].colluser==th.username){//我的收藏单词添加用户标识
                            userword.push(th.data[i]);
                        }
                    }
                    th.wordlength=userword.length;
                    if(userword.length==0){//没数据时显示提示
                        th.isdatames=true;
                        th.datas=[];
                    }else{
                        th.isdatames=false;
                        th.datas=userword;//赋值给显示页面的数据
                    }

                }else{//根据点击的username字符改变datas
                    th.wordtitle=`${username}的单词`;
                    // console.log(th.data)
                    for(let i=0;i<th.data.length;i++){//从原始数据中提取和要求的数据
                        if(th.data[i].user==username){
                            userword.push(th.data[i]);
                        }
                    }
                ...
                console.log(th.datas,th.data);
                }
            });
            connect.$on('letterworddata',function(e){//letterNav数据
                // console.log(th.data,th.data);//dats输出到页面，data从数据库和获取到未修改过
                let userword=[];
                if(e=="#"){
                    th.wordtitle="全部单词";
                    // th.datas=th.data;
                    th.wordlength=th.data.length;
                    if(th.data.length==0){//没数据时显示提示
                        th.isdatames=true;
                        th.datas=[];
                    }else{
                        th.isdatames=false;
                        th.datas=th.data;//赋值给显示页面的数据
                    }
                }else{//根据点击的e字符改变datas
                     th.wordtitle=`首字母${e}的单词`;
                    for(let i=0;i<th.data.length;i++){//从原始数据中提取和要求的数据
                        if(th.data[i].english.toUpperCase().indexOf(e)==0){//判断首字母 toUpperCase转大写
                            userword.push(th.data[i]);
                        }
                    }
                    th.wordlength=userword.length;//单词个数
                    if(userword.length==0){//没数据时显示提示
                        th.isdatames=true;
                        th.datas=[];
                    }else{
        ...
        updateword(){//添加单词后更新列表
            let th=this;
            connect.$on('updateword',function(number,english,chinese1,chinese2,username,dynamicTags){
                if(number==200){
                    th.$http.get("/getword").then((res)=>{//获取全部单词
                        console.log(res.data);
                        let addword;
                        for(let i=0;i<res.data.length;i++){//定位刚刚添加的单词的id并赋值给对象里
                            if(i==res.data.length-1 && res.data[i].english==english){
                                let date=new Date();
                                addword={
                                    chinese1:chinese1,
                                    chinese2:chinese2,
        ...
        mycollection(id,index){//我的收藏
            ...
            this.$http.get(`/mycollection?wordid=${id}&username=${this.username}`).then((res)=>{
                ...
                if(res.data.number==404){
                    // for(let i=0; i<this.data.length;i++){
                    //     if(this.data[i].id==id){
                    //         this.data[i].collection='0';
                    //         this.data[i].colluser='';
                    //         // alert("dd");
                    //     }
                    //     // this.datas=this.data;
                    // }
                    this.datas[index].collection='0';
                    this.datas[index].colluser='';
                    // console.log(this.datas);
                    this.$notify({
                        title:"取消收藏",
                        duration:1000,
                    });
                }else if(res.data.number==200){
                    this.$notify({
                        title:"收藏成功",
                        duration:1000,
                    });
                    ...
                    //         this.data[i].colluser=this.username; 
                    //     }
                    //     // this.datas=this.data;
                    // }
                    this.datas[index].collection='1';
                    this.datas[index].colluser=this.username;    
                }
